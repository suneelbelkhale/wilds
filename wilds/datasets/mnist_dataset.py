...
class Camelyon17Dataset(WILDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official'):
        ...
        elif self._split_scheme == 'domain':
            # we move half the validation slides (val_center) to training
        ...
        self._metadata_array = torch.stack(
            (torch.LongTensor(centers),
             ...
             self._y_array,
             torch.LongTensor((val_center_mask | test_center_mask).values)),  # domain
            dim=1)
